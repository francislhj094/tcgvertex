...
// Migrate to Firestore helper
export const migrateLocalAlertsToFirestore = async (userId, localAlerts) => {
  // This function is imported from firestoreAlerts.js
  // We're just re-exporting it here for convenience
  const { migrateLocalAlertsToFirestore: migrate } = await import('./firestoreAlerts.js');
  return migrate(userId, localAlerts);
};
